...
author:
  - name: "Subdepto. de Instalación y Gestión Educativa"
    affiliation: "División de Desarrollo Educativo"
date: today
...
Los datos de cuentas inactivas presentan una frecuencia irregular de actualización, por lo que la información presentada en el presente reporte puede estar desfasada. 

Adicionalmente, es posible que se contabilicen establecimientos sin cuenta activa pero que estén gestionado las listas de espera, esto debido a que se puede estar utilizando un usuario que ya no se encuentra trabajando en el establecimiento. 

Por lo anterior, es que todo establecimiento que no tenga una cuenta creada para el actual director/a o encargado/a SAE aparecerá categorizado como establecimiento inactivo.
:::
...
#| results: "asis"
  ...
  listado_cuentas_table <- data.frame(
  establecimiento = c(
    'ESCUELA ADELAIDA MIGUIELES SOTO - 4994',
    'ESCUELA BASICA LA HUERTA - 11676',
    'ESCUELA BASICA PATAGUAL - 5006',
    'ESCUELA ISIDORA GOYENECHEA - 4953'
  ),
  `Lista de espera` = c(0, 0, 1, 2),
  correo_director = c(
    '',  # Dato faltante
    '',  # Dato faltante
    'peyoflores@gmail.com',
    'patricio.ramirez@gmai.com'
  ),
  correo_encargado_sae = c(
    'escuela.adelaidamiguielessoto@gmai.com',
    'nelsonespejogutierrez@gmail.com',
    'peyoflores@gmail.com',
    'trinidadesperanzamedina@gmail.com'
  ),
  fecha_creacion_director = c(
    '',  # Dato faltante
    '',  # Dato faltante
    '27-02-2025',
    '16-05-2025'
  ),
  fecha_creacion_encargado_sae = c(
    '22-08-2025',
    '27-02-2025',
    '27-02-2025',
    '27-02-2025'
  ),
  check.names = FALSE,  # Para mantener el nombre "Lista de espera" con espacio
...
datos_fig_1 <-datos_fig_1 %>%  mutate(nombre_ejemplo="Ejemplo")

#print(datos_fig_1)

datos_fig_1 %>%
    mutate(nombre_slep = if_else(nombre_slep == params$slep, nombre_slep, "")) %>%
  mutate(nombre_slep= if_else(nombre_slep != "", "ejemplo",nombre_slep)) %>% 
...
En esta sección se presenta el comportamiento de las vacantes sin asignar (VAS) durante el periodo de Enero a Octubre 2025 para cada comuna del territorio, los datos se obtuvieron con una frecuencia semanal, actualizando el visor todos los lunes del año.
...
temp_2 <- temp_2 %>% 
  ...
  mutate(comuna = case_when(
    comuna == "Coronel" ~ "Comuna 1",
    comuna == "San Pedro de la Paz" ~ "Comuna 2",
    comuna == "Lota" ~ "Comuna 3",
    comuna == "Santa Juana" ~ "Comuna 4",
    TRUE ~ "Otro"  # Mantener valor original si no coincide
  )) %>% 
...
n_filas <- nrow(temp3)
  ...
  temp3 <- temp3 %>% mutate(Comuna = case_when(
    Comuna == "Coronel" ~ "Comuna 1",
    Comuna == "San Pedro de la Paz" ~ "Comuna 2",
    Comuna == "Lota" ~ "Comuna 3",
    Comuna == "Santa Juana" ~ "Comuna 4",
...
listado_ael_rbd <- params$ael_actual
listado_ael_rbd = as.data.frame(listado_ael_rbd)
listado_ael_rbd <- listado_ael_rbd %>% 
  mutate(
    comuna = case_when(
      row_number() %% 4 == 1 ~ "Comuna1",
      row_number() %% 4 == 2 ~ "Comuna2",
      row_number() %% 4 == 3 ~ "Comuna3",
      row_number() %% 4 == 0 ~ "Comuna4"
    ),
    rbd = 10000 + row_number() * 100,  # RBD de 5 dígitos
    nombre_ee = case_when(
      row_number() == 1 ~ "LICEO CENTRAL",
      row_number() == 2 ~ "ESC HORIZONTE",
      row_number() == 3 ~ "LICEO PROGRESO",
      row_number() == 4 ~ "COLEGIO AURORA",
      row_number() == 5 ~ "ESC LAS FLORES",
      row_number() == 6 ~ "LICEO ESTRELLA",
      TRUE ~ paste0("ESCUELA ", row_number())
    )
  )
